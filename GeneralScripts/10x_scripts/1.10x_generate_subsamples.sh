...
# 10x downsampling for paired-end FASTQs

# Directories
INPUT_DIR="/data2/gonzalo2/alvaro_data2/EMSeq_cov/15x/raw_15x"
OUTPUT_DIR="/data2/gonzalo2/alvaro_data2/EMSeq_cov/10x/raw_fastq_10x_subsampled"

# Fraction for 10x from 15x (exact calculation)
FRACTION=$(echo "scale=6; 10/15" | bc)
...
docker run --rm \
  ...
  staphb/seqtk \
  bash -c '
    echo "Starting 10x downsampling from 15x..."
    ...
    for R1 in /input/*_R1.fastq.gz; do
      ...
      echo "Downsampling $BASENAME to 10x coverage"
    ...
    echo "10x downsampling complete in container!"
  '

echo "10x downsampling complete! Output in $OUTPUT_DIR"
