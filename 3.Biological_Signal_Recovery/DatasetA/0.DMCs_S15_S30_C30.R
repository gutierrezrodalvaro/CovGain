# With this scrip  we determine which CpG is each methodology clasiffiying as DMCs
# Data for Figure 3
# 09/06/2026
...
# STD15x_meth
S15_diff <- calculateDiffMeth(STD15x_meth, mc.cores = 4)
S15_DMCs <- getMethylDiff(S15_diff, qvalue = 0.05, difference = 15)

# CTM15x_meth
C15_diff <- calculateDiffMeth(CTM15x_meth, mc.cores = 4)
C15_DMCs <- getMethylDiff(C15_diff, qvalue = 0.05, difference = 15)

# CTM30x_meth
C30_diff <- calculateDiffMeth(CTM30x_meth, mc.cores = 4)
C30_DMCs <- getMethylDiff(C30_diff, qvalue = 0.05, difference = 15)

# CTM15x_meth
S30_diff <- calculateDiffMeth(STD30x_meth, mc.cores = 4)
